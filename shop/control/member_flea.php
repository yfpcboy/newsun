...
//				echo json_encode(array('state'=>'false','message'=>$lang['iswfupload_upload_pic_fail']));
	...
	private function profile_menu($menu_type,$menu_key='',$array=array()) {
		...
		switch ($menu_type) {
			case 'goods':
				$menu_array	= array(
					1=>array('menu_key'=>'flea_list',	'menu_name'=>二手列表,			'menu_url'=>'index.php?act=member_flea')
				);
				break;
			case 'add_goods':
				$menu_array = array(
					1=>array('menu_key'=>'flea_list',	'menu_name'=>二手列表,			'menu_url'=>'index.php?act=member_flea'),
					2=>array('menu_key'=>'goods_add',	'menu_name'=>发布二手,	'menu_url'=>'index.php?act=member_flea&op=add_goods')
				);
				break;
			case 'favorites':
				$menu_array = array(
					1=>array('menu_key'=>'favorites',	'menu_name'=>编辑二手,	'menu_url'=>'index.php?act=member_flea&op=favorites')
				);
		}
		if(!empty($array)) {
			$menu_array[] = $array;
